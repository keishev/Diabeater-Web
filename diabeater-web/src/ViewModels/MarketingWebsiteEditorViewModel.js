// src/ViewModels/MarketingWebsiteEditorViewModel.js //admin
...
const repository = new MarketingContentRepository(db, null, storage);
...
export async function uploadAPKFile(file) {
    try {
        const downloadURL = await repository.uploadAPK(file);
        console.log(`MarketingWebsiteEditorViewModel: Uploaded APK: ${file.name}`);
        return downloadURL;
    } catch (error) {
        console.error(`MarketingWebsiteEditorViewModel: Failed to upload APK:`, error);
        throw error;
    }
}
 ...
 * @throws {Error}
 */
export async function deleteAPKFile(fileName) {
    try {
        await repository.deleteAPK(fileName);
        console.log(`MarketingWebsiteEditorViewModel: Deleted APK: ${fileName}`);
    } catch (error) {
        console.error(`MarketingWebsiteEditorViewModel: Failed to delete APK:`, error);
        throw error;
    }
}
