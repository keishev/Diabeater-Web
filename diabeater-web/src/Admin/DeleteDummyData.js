...
const ImprovedDeleteDummyData = () => {
  ...
  const deleteDummyData = async () => {
    ...
    try {
      addLog('🗑️ Starting comprehensive dummy data deletion...');
      ...
      if (userDeletePromises.length > 0) {
        ...
        addLog('ℹ️ No dummy users found in user_accounts');
      }
      ...
      if (appDeletePromises.length > 0) {
        ...
        addLog('ℹ️ No dummy applications found in nutritionist_application');
      }
      ...
      for (const collectionName of otherCollections) {
        try {
          ...
          if (deletePromises.length > 0) {
            ...
            addLog(`ℹ️ No dummy records found in ${collectionName}`);
          }
          
        } catch (error) {
          addLog(`⏭️ Collection ${collectionName} doesn't exist or can't be accessed, skipping...`);
        }
      }
