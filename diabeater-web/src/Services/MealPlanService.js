...
    
    /**
     * Fetches the top meal plans, ordered by saveCount (mapped to 'likes' field)
     * in descending order.
     */
    async getPopularMealPlans() {
        // The user's request is to sort by 'saveCount', which in the provided code
        // is represented by the 'likes' field (which is a number).
        // A reasonable limit is applied to keep the list manageable.
        const q = query(
            collection(this.db, 'meal_plans'),
            orderBy('likes', 'desc'),
            limit(20) 
