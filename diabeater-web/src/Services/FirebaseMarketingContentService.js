// src/Services/FirebaseMarketingContentService.js //admin
...
class FirebaseMarketingContentService {
    constructor(firestoreInstance, authInstance, storageInstance) {
        if (!firestoreInstance) {
            throw new Error("FirebaseMarketingContentService requires a Firestore instance.");
        }
        this.db = firestoreInstance;
        this.auth = authInstance; 
        this.storage = storageInstance; // Firebase Storage instance
        ...
        console.log("FirebaseMarketingContentService initialized with Firestore and Storage.");
    }
    ...
    async uploadAPK(file) {
        ...
        try {
            ...
            const storageRef = ref(this.storage, `assets/${file.name}`);
            
            // Upload file
            console.log("Uploading APK file to Firebase Storage...");
            ...
            // Update the marketing content with new APK URL
            await this.updateContentField('apkDownloadUrl', downloadURL);
            await this.updateContentField('apkFileName', file.name);
    ...
    /**
     * Delete the current APK file from Firebase Storage
    ...
    async deleteAPK(fileName) {
        ...
        try {
            const storageRef = ref(this.storage, `assets/${fileName}`);
            await deleteObject(storageRef);
            console.log("APK file deleted successfully from Firebase Storage.");
            
            // Reset APK URL in content
            await this.updateContentField('apkDownloadUrl', "/assets/Diabeater.apk");
            await this.updateContentField('apkFileName', "Diabeater.apk");
        } catch (error) {
